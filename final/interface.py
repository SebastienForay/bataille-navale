...
class Interface():
	def __init__(self, pNumber_init, pseudo_init, connectionServer_init, thread_init, receiv_init, pLocal_init, pOnline_init):
		...
		self.thread = thread_init
		self.threadRes = receiv_init
		self.pLocal = pLocal_init
		self.pOnline = pOnline_init
	...
	def bigFrame(self):
		...
		Label(cadre, text="Joueur : " + str(self.pPseudo), bg="white").pack(padx=10, pady=10)
	...
	def initTab(self, root):
		...
		ok = Button(cadre, text = "ok", command=self.waitShip)
		ok.pack()
	...
	def placeBateau(self,event):
		...
		if self.nbShip == 1:
			...
			if x > 50 and x < 550 and y > 50 and y < 550:
				...
				if self.nbShip < 6:
					self.pLocal.tableShips.append(self.tmpShip)
					self.tmpShip.insertShip()
					self.connectionServer.send(str.encode("/createship " + str(self.pNumber) + str(self.oldLen) + str(self.saveX // 50) + str(self.saveY // 50) + str(self.pos)))
	...
	def beginBateau(self):
		text = "Joueur " + str(self.pPseudo) + ": veuillez placer vos bateaux"
	...
	def waitShip(self):
		...
		photo = PhotoImage(file="wait.gif", format="gif -index 2")
		label = Label(wait, image = photo).pack()
		wait.pack(fill="both", expand=True, padx=20, pady=20)
		#root.tag_raise(wait)
		wait.lift()
		self.wait = wait"""
		self.pLocal.tableShips.append(self.tmpShip)
	...
	def wait(self, message):
		...
		photo = PhotoImage(file="wait.gif", format="gif -index 2")
		label = Label(win, win = photo).pack()
		return win
